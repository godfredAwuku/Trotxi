...
- CD (deploy.yml): fires after CI succeeds on main — staging deploys automatically via the Render API (trigger + poll until live in .github/scripts/render-deploy.sh) then smoke-tests /healthz + /readyz; production runs the same way but behind a manual-approval GitHub environment. Dormant until the `DEPLOY_ENABLED` repo variable is true (one-time Render setup in docs/DEPLOY.md); production stage skipped until `RENDER_PRODUCTION_SERVICE_ID` is set. render.yaml defines trotxi-api-staging (autoDeploy off) with a commented production block
